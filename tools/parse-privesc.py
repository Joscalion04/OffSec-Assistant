...
LINPEAS_SUID_RE = re.compile(
    r'(-rws[rwx-]{6}|--s[rwx-]{6})\s+\S+\s+\S+\s+(\S+)',
    re.IGNORECASE
)

LINPEAS_SUDO_RE = re.compile(
    r'(\w+)\s+ALL=.*NOPASSWD.*',
    re.IGNORECASE
)

LINPEAS_WRITABLE_RE = re.compile(
    r'(?:Writable|world.?writable)\s+(?:path|dir|file):\s*(\S+)',
    re.IGNORECASE
)

LINPEAS_CRON_RE = re.compile(
    r'(?:/etc/cron\S*|crontab).*?(\S+\.(?:sh|py|pl|rb|php))',
    re.IGNORECASE
)

LINPEAS_KERNEL_RE = re.compile(
    r'Linux version\s+([\d.]+)',
    re.IGNORECASE
)
...
WINPEAS_SECTION_RE = re.compile(
    r'={10,}\s*(.+?)\s*={10,}',
    re.IGNORECASE
)

WINPEAS_HIGH_RE = re.compile(
    r'(?:\[!\]|\[!!!\]|CRITICAL|Found interesting)',
    re.IGNORECASE
)

WINPEAS_CRED_RE = re.compile(
    r'(?:Password|Credential|Secret|Token)\s*[=:]\s*(\S+)',
    re.IGNORECASE
)

WINPEAS_ALWAYSINSTALLELEVATED_RE = re.compile(
    r'AlwaysInstallElevated.*?(?:PRESENT|enabled|1)',
    re.IGNORECASE
)

WINPEAS_AUTOLOGON_RE = re.compile(
    r'DefaultPassword\s*[=:]\s*(\S+)',
    re.IGNORECASE
)

WINPEAS_UNQUOTED_RE = re.compile(
    r'Unquoted Service Path.*?:\s*(.+)',
    re.IGNORECASE
)

WINPEAS_WRITABLEREG_RE = re.compile(
    r'(?:Writable|Modifiable)\s+(?:Registry|Service)\s*:\s*(\S+)',
    re.IGNORECASE
)

WINPEAS_MODIFIABLE_RE = re.compile(
    r'Modifiable\s+(?:binary|path|service)\s*:\s*(\S+)',
    re.IGNORECASE
)
...
def detect_tool(content: str) -> str:
    ...
    if 'PEASS-ng' in content or 'linpeas' in content.lower() or 'Linux Privesc' in content:
        return 'linpeas'
    if 'winpeas' in content.lower() or 'Windows Privesc' in content or 'WinPEAS' in content:
        return 'winpeas'
...
def generate_finding_md(findings: dict, engagement_dir: Path) -> str:
    ...
    cvss_map = {"Critical": "9.0", "High": "7.5", "Medium": "5.5", "Low": "3.5", "Info": "1.0"}
    cvss = cvss_map.get(severity, "5.0")

    sections = [
        f"# FIND-PRIVESC — Post-Explotacion: Privilege Escalation Vectors",
        f"",
        f"**Fecha:** {now}",
        f"**Herramienta:** {tool}",
        ...
        f"Se identificaron vectores potenciales de escalacion de privilegios.",
        f"",
        f"---",
        f"",
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Parser automático de linPEAS/winPEAS"
    )
    parser.add_argument("engagement_dir", help="Directorio del engagement (para sanitizacion DLP)")
    ...
    if tool == "linpeas":
        ...
        print("[WARN] Herramienta no detectada, intentando parse como linPEAS", file=sys.stderr)
        findings = parse_linpeas(lines)
    ...
    if findings.get("always_install_elevated"):
        print(f"  [!] AlwaysInstallElevated: HABILITADO")

    print(f"\n  Ejecutar con --finding para generar finding_privesc.md")
    print(f"  Ejecutar con --json para output detallado")
