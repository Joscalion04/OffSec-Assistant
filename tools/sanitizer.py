...
CREDENTIAL_PATTERNS = [
    ...
    # Hashes Unix (crypt)
    re.compile(r'\$(?:1|2[aby]|5|6)\$[A-Za-z0-9./]{1,16}\$[A-Za-z0-9./]{22,86}'),
    # NTLM / LM (32 hex chars) - solo si están en contexto de hash
    re.compile(r'\b(?:NTLM|LM|MD5|SHA1|SHA256|hash)[:\s]+([A-Fa-f0-9]{32,64})\b', re.IGNORECASE),
...
class DLPSanitizer:
    ...
    def _token(self, category: str, value: str, prefix: str) -> str:
        ...
        self._map['counters'][counter_key] = self._map['counters'].get(counter_key, 0) + 1
